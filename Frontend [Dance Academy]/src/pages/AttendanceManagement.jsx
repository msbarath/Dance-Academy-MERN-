...
function AttendanceManagement() {
    ...
    function startEdit(record) {
        const sid = record.student
            ? (typeof record.student === "object" ? String(record.student._id) : String(record.student))
            : "";
        setEditing(record);
        setForm({ studentId: sid, date: record.date, status: record.status });
        setErrors({});
        setApiErr("");
    }
    ...
    const { displayed, presentCount, absentCount, todayPresent, todayAbsent, filteredPresent, filteredAbsent } = useMemo(() => {
        let presentCount = 0, absentCount = 0, todayPresent = 0, todayAbsent = 0, filteredPresent = 0, filteredAbsent = 0;
        const displayed = [];
        for (const a of attendance) {
            const isPresent = a.status === "Present";
            if (isPresent) presentCount++; else absentCount++;
            if (a.date === TODAY) { if (isPresent) todayPresent++; else todayAbsent++; }
            if (!filterDate || a.date === filterDate) {
                displayed.push(a);
                if (isPresent) filteredPresent++; else filteredAbsent++;
            }
        }
        return { displayed, presentCount, absentCount, todayPresent, todayAbsent, filteredPresent, filteredAbsent };
    ...
    return (
        <div className="panel-page">
            ...
            <div className="panel-stats-row">
                ...
                <StatCard label={filterDate ? "Present (filtered)" : "All Present"} value={filterDate ? filteredPresent : presentCount} variant="success" />
                <StatCard label={filterDate ? "Absent (filtered)"  : "All Absent"}  value={filterDate ? filteredAbsent  : absentCount}  variant="danger" />
                            ...
                            {!editing && (
                                <>
                                    ...
                                    <FormField label="Date" error={errors.date}>
                                        <input type="date" name="date" max={TODAY} value={form.date} onChange={handleChange} onBlur={handleBlur} className={errors.date ? "input-error" : ""} />
                                    </FormField>
                                </>
                            )}
                            {editing && (
