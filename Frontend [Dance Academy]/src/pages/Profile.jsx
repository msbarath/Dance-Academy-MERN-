...
function Profile() {
    ...
    function handleBlur(e) {
        const { name, value } = e.target;
        if (name === "phone") {
            setErrors(p => ({ ...p, phone: validate("phone", value, false) }));
        } else {
    ...
    async function handleSubmit(e) {
        ...
        const newErrors = {
            ...
            phone:     validate("phone",     form.phone, false),
        };
        setErrors(newErrors);
        ...
        if (result.success) {
            ...
            setApiErr(result.message || "Failed to update profile.");
        }
    }
    ...
    return (
        <div className="panel-page">
            ...
            <div className="panel-form-box" style={{ maxWidth: 520 }}>
                ...
                {success && (
                    <div role="status" style={{ background: "#e8f9f0", color: "#1a7a4a", borderRadius: 8, padding: "10px 14px", fontSize: 13, fontWeight: 500, marginBottom: 14 }}>
                        {success}
                    </div>
                )}
                {apiErr && <div className="panel-error" role="alert">{apiErr}</div>}
