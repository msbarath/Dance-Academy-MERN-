...
function FeeManagement() {
    ...
    function startEdit(fee) {
        setEditing(fee);
        setForm({ studentId: fee.student, amount: String(fee.amount), month: fee.month, status: fee.status });
        setErrors({});
        setApiErr("");
    }
    ...
    async function handleUpdate(e) {
        ...
        setSaving(true);
        try {
            const r = await feesApi.update(editing._id, { amount: Number(form.amount), month: form.month, status: form.status });
    ...
    const totalCollected = useMemo(() => fees.filter(f => f.status === "Paid").reduce((s, f) => s + Number(f.amount || 0), 0), [fees]);
    const totalPending   = useMemo(() => fees.filter(f => f.status === "Pending").reduce((s, f) => s + Number(f.amount || 0), 0), [fees]);
                        ...
                        <div className="panel-form-row">
                            ...
                            {editing && (
                                <div style={{ padding: "8px 0", fontWeight: 600 }}>
                                    {editing.studentName} &mdash; {editing.course}
                                </div>
                            )}
